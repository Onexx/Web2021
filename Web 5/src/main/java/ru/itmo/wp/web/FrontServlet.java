...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        String lang;
        if ((lang = request.getParameter("lang")) != null && lang.matches("[a-z]{2}")) {
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method curMethod : clazz.getDeclaredMethods()) {
                if (curMethod.getName().equals(route.getAction()) &&
                        curMethod.getReturnType().equals(Void.TYPE) &&
                        Arrays.stream(curMethod.getParameterTypes())
                                .allMatch(parameterType -> (parameterType == Map.class || parameterType == HttpServletRequest.class)) &&
                        (method == null || curMethod.getParameterCount() < method.getParameterCount())
                ) {
                    method = curMethod;
                }
            }
